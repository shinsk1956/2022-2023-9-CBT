...
const App: React.FC = () => {
  ...
  // 학습/시험 모드 관리
  const [currentMode, setCurrentMode] = React.useState<'LEARNING' | 'EXAM'>('LEARNING');
        ...
        try {
          ...
          try {
            ...
            return {
              ...
              // [수정됨] 여기에 잘못 끼어있던 const res = ... 코드를 삭제했습니다.
              data: questions
  ...
  React.useEffect(() => {
    ...
    if (appMode === 'USER' && currentMode === 'EXAM' && selectedUserRecordId && !examSubmitted) {
  ...
  React.useEffect(() => {
    if (currentMode !== 'EXAM' || !selectedUserRecordId) {
      setExamTimer(0);
    }
  ...
  const handleRetryDojo = () => {
    // This is for retrying within Dojo mode
    ...
    if (stillWrong.length === 0) {
      ...
      setRetryMode(false);
      setRetryIds(new Set());
      setSelectedUserRecordId(null); // Go back to the list view
      return;
    }
      ...
      prev.map(record => {
        if (record.id === selectedUserRecordId) {
          const newData = record.data.map(q => {
            // Reset only the questions that are still wrong
    ...
    setNavPage(0); // Reset pagination to the start
  ...
  const handleModeChange = (mode: 'LEARNING' | 'EXAM') => {
    if (mode === currentMode) return;
    if (mode === 'EXAM') {
      if (answeredCount > 0 && !examSubmitted && !retryMode && !window.confirm('시험 모드로 전환하면 현재 문제 풀이 기록이 초기화됩니다. 계속하시겠습니까?')) {
        return;
      }
  ...
  return (
    <div className={`min-h-screen pb-20 transition-all ${retryMode ? 'bg-orange-50' : 'bg-slate-50'}`}>
            ...
            {activeRecordId && (
              <div className="animate-fade-in space-y-10">
                {currentQuestions.map(q => (
                  <QuestionCard key={q.id} question={q} isAdmin={appMode === 'ADMIN'} onUpdate={handleUpdateQuestion} onDelete={(id) => setUploadedFiles(prev => prev.map(f => ({ ...f, data: f.data.filter(mq => mq.id !== id) })))} activeAudioInfo={activeAudioInfo} setActiveAudioInfo={setActiveAudioInfo} isExamMode={appMode === 'USER' && currentMode === 'EXAM'} showAnswers={appMode === 'ADMIN' || (currentMode === 'LEARNING' && q.userAnswer !== undefined) || examSubmitted}/>
                ))}
                {appMode === 'USER' && currentMode === 'EXAM' && !examSubmitted && (
      ...
      {showExamResultModal && activeRecordId && <StatsModal data={uploadedFiles} onClose={() => setShowExamResultModal(false)} examRecordId={activeRecordId} onStartDojo={startDojo}/>}
      {recordToDelete && (
